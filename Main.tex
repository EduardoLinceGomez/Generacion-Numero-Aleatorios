...
% \newcommand{\chapteropening}[3][]{%
%   {%
%     \renewcommand{\LettrineFontHook}{\bfseries\LARGE}% letra capitular
%     \renewcommand{\LettrineTextFont}{\normalfont}% texto junto a la capitular SIN small caps
%     \lettrine[lines=2,
%             lhang=0.0, 
%             lraise=0.7,
%             findent=\parindent,
...
\newcommand{\ChapterLabel}[1]{%
  \begin{tikzpicture}[baseline={(num.base)}]
    \node[inner sep=0pt,minimum width=1.4cm,minimum height=1.7cm,fill=olive] (num) {\color{white}\bfseries\fontsize{38}{42}\selectfont #1};
    \node[anchor=east,rotate=90,inner sep=4pt,minimum width=1.6cm,minimum height=0.7cm,fill=olive] at ([xshift=-0.5cm,yshift=1.2cm]num.west) {\color{white}\bfseries\footnotesize\MakeUppercase{Capítulo}};
  \end{tikzpicture}%
}
...
%   {\normalfont\huge\bfseries} % fuente grande y negrita
%   {\color{dark}\fontsize{50}{60}\selectfont\thechapter} % número del capítulo, muy grande y en rojo
%   {1ex} % separación entre número y título
